...
foreach ( $Cities as $key=>$City ) {
  echo  ( $key + 1 ), ':', $City->engName, "\n";

  for ( $nDate = 1; $nDate < 8;  $nDate++) {
    ...
    if ($weathers[1]->temperature != NULL)
      $weathers[0]->NightWeather = $weathers[1];
    if ($weathers[2]->temperature != NULL)
      $weathers[0]->MorningWeather = $weathers[2];
    if ($weathers[3]->temperature != NULL)
      $weathers[0]->DayWeather = $weathers[3];
    if ($weathers[4]->temperature != NULL)
